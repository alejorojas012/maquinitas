      ...
      <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end', height: 14 }}>
        {[25, 50, 75, 100].map((threshold, i) => (
          <div key={i} style={{
            ...
            height: 4 + i * 3,
            borderRadius: 2,
            background: pct >= threshold ? color : '#1e293b',
          }} />
        ))}
      </div>
...
export default function Dashboard() {
  ...
  return (
    <div style={{ background: '#060d1a', minHeight: '100vh', color: '#fff', fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0, color: '#fff' }}>Maquinitas</h1>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button onClick={reload}
              style={{ padding: '6px 16px', borderRadius: 8, background: '#22c55e', color: '#000', border: 'none', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
              ↻ Actualizar
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10, marginBottom: 20 }}>
          <div style={{ background: '#0d1929', border: '1px solid #1e293b', borderRadius: 12, padding: '14px 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              ...
              <button onClick={() => setHideAmounts(!hideAmounts)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569', fontSize: 14, padding: 0 }}>
                {hideAmounts ? '👁️' : '🙈'}
              </button>
            </div>
          ...
          <div style={{ background: '#0d1929', border: '1px solid #1e293b', borderRadius: 12, padding: '14px 16px' }}>
            ...
            <p style={{ fontSize: 22, fontWeight: 800, color: '#22c55e', margin: 0 }}>{fmt(parseInt(bestMonth?.offlineOutCoinSum || '0'))} <span style={{ fontSize: 12, fontWeight: 400, color: '#475569' }}>fichas</span></p>
          </div>
          ...
          <div style={{ background: '#0d1929', border: '1px solid #1e293b', borderRadius: 12, padding: '14px 16px' }}>
            ...
            <p style={{ fontSize: 22, fontWeight: 800, color: '#22c55e', margin: 0 }}>{fmt(parseInt(bestYesterday?.offlineOutCoinSum || '0'))} <span style={{ fontSize: 12, fontWeight: 400, color: '#475569' }}>fichas</span></p>
          </div>
        </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 10 }}>
                {filteredMachines.map((m, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      background: '#0d1929',
                      border: `1px solid ${m.online ? '#1a3a2a' : '#3a1a1a'}`,
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                        ...
                        <span style={{
                          ...
                          background: m.online ? '#0d2818' : '#2a0d0d',
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 10 }}>
                        <div style={{ background: '#0a1525', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
                          <p style={{ fontSize: 9, color: '#475569', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: 0.5 }}>Fichas Hoy</p>
                          <p style={{ fontSize: 20, fontWeight: 800, color: '#22c55e', margin: 0 }}>{fmt(tokensHoy)}</p>
                        </div>
                        <div style={{ background: '#0a1525', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
                          <p style={{ fontSize: 9, color: '#475569', margin: '0 0 2px', textTransform: 'uppercase', letterSpacing: 0.5 }}>Fichas Mes</p>
